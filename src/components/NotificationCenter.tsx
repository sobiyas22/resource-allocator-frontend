...
export function NotificationCenter() {
  ...
  async function fetchNotifications() {
    try {
      ...
      // but structure is ready for when it's implemented
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'booking_rejected': return <XCircle className="w-4 h-4 text-rose-600" />
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="icon" className="relative">
          ...
          {unreadCount > 0 && (
            <Badge 
              className="absolute -top-2 -right-2 h-5 w-5 p-0 flex items-center justify-center bg-rose-500 text-white text-xs"
            >
        ...
        <div className="max-h-96 overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            notifications.map((notification) => (
              <DropdownMenuItem
                key={notification.id}
                className={`p-4 cursor-pointer ${!notification.is_read ? 'bg-indigo-50' : ''}`}
